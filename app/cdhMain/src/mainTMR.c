...
#include "../../spiFlashTest/src/flash_provision_tmr.h"
...
static const struct gpio_dt_spec     fram_cs_0 =
    GPIO_DT_SPEC_GET_BY_IDX(DT_NODELABEL(spi2), cs_gpios, 0);
static const struct gpio_dt_spec     fram_cs_1 =
    GPIO_DT_SPEC_GET_BY_IDX(DT_NODELABEL(spi2), cs_gpios, 1);
static const struct gpio_dt_spec     fram_cs_2 =
    GPIO_DT_SPEC_GET_BY_IDX(DT_NODELABEL(spi2), cs_gpios, 2);
static const struct gpio_dt_spec     scope_pin = GPIO_DT_SPEC_GET(LED1_NODE, gpios);
...
K_THREAD_STACK_DEFINE(tmr_stack,       STACK_SIZE);
...
static struct k_thread tmr_thread_data;
...
static void init_fram_dev(struct fram_dev *dev, const char *name,
                          const struct gpio_dt_spec *cs);
static void tmr_init_and_provision(void);
                          ...
                          const struct gpio_dt_spec *cs)
{
    dev->name = name;
    dev->spi_bus = spi_bus;
    /* Use slowest driver-legal SPI rate for this board clocking. */
    dev->spi_cfg.frequency = 625000;
...
static void tmr_init_and_provision(void)
{
    if (!device_is_ready(spi_bus)) {
        LOG_ERR("SPI bus not ready for TMR");
        return;
    }
    if (!gpio_is_ready_dt(&fram_cs_0) ||
        !gpio_is_ready_dt(&fram_cs_1) ||
        !gpio_is_ready_dt(&fram_cs_2) ||
        !gpio_is_ready_dt(&scope_pin)) {
        LOG_ERR("FRAM CS or scope GPIO not ready");
        return;
    }

    gpio_pin_configure_dt(&scope_pin, GPIO_OUTPUT_INACTIVE);

    init_fram_dev(&fram_a, "FRAM_A", &fram_cs_0);
    init_fram_dev(&fram_b, "FRAM_B", &fram_cs_1);
    init_fram_dev(&fram_c, "FRAM_C", &fram_cs_2);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (err) {
            LOG_ERR("RDID FRAM_%s failed err=%d", labels[i], err);
            continue;
        }
    ...
    if (!device_is_ready(flash_dev_ptr)) {
    ...
    struct flash_tmr_scrub_result verify_result;
    err = flash_tmr_scrub(flash_dev_ptr, &fram_a, &fram_b, &fram_c, &verify_result);
    if (err) {
        LOG_ERR("Post-provision scrub failed: %d", err);
        return;
    }
    if (verify_result.flash_mismatches > 0U) {
        LOG_ERR("Post-provision mismatches: %u", verify_result.flash_mismatches);
        return;
    }

    printk("POST-PROVISION VERIFY: OK  (%u chunks)\n", verify_result.chunks_checked);
    tmr_ready = true;
...
void tmr_thread(void *a, void *b, void *c)
{
    LOG_INF("tmr_thread started");
    k_sem_take(&scrub_start_sem, K_FOREVER);
    printk("TMR scrub thread running (interval %u ms)\n", (unsigned)SCRUB_INTERVAL_MS);
    ...
    while (1) {
        ...
        if (tmr_ready &&
            ...
            if ((scrub_count % INJECT_EVERY_N) == 0U) {
                int target_idx = (scrub_count / INJECT_EVERY_N) % 3U;
                (void)inject_bitflip(fault_targets[target_idx], INJECT_ADDR);
            }

            struct flash_tmr_scrub_result result;
            gpio_pin_set_dt(&scope_pin, 1);
            int err = flash_tmr_scrub(flash_dev_ptr, &fram_a, &fram_b, &fram_c, &result);
            gpio_pin_set_dt(&scope_pin, 0);

            if (err) {
                printk("SCRUB #%u ERROR: %d\n", scrub_count, err);
            } else if (result.flash_mismatches > 0U) {
            ...
            LOG_WRN("TMR scrub skipped: provisioning incomplete");
        }
    }
}
 ...
 int main(void)
 {
    ...
    k_thread_create(&tmr_thread_data, tmr_stack, STACK_SIZE,
                    tmr_thread, NULL, NULL, NULL,
                    PRIO_TELEMETRY + 1, 0, K_NO_WAIT);

    tmr_init_and_provision();
     setup_mode_init();
