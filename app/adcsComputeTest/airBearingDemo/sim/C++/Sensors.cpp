...
SensorsClass::SensorsClass()
    : // Initialize Members
    epoch_time(Param::SimTime::epoch_timestamp),
    current_time(Param::SimTime::epoch_timestamp),
    Ts(Param::SimTime::Ts),
    mu_E(Param::Earth::mu_E),
    r_E(Param::Earth::r_E),
    // accelerometer
    beta_a(Param::Sensors::beta_a),
    sigma_a(Param::Sensors::sigma_a),
    // gyro
    sigma_gyro(Param::Sensors::sigma_gyro),
    beta_gyro(Param::Sensors::beta_gyro),
    // star tracker
    star_update(Param::Sensors::star_update),
    y_star_previous(Param::Orbit::InitialState.q0),
    sigma_star(Param::Sensors::sigma_star),
    beta_star(Param::Sensors::beta_star),
    small_angle_tol(Param::Sensors::small_angle_tol),
    T_star(Param::Sensors::T_star),
    previous_star_update(0.0),
    // sun sensor
    I_max(Param::Sensors::I_max),
    beta_css(Param::Sensors::beta_css),
    sigma_css(Param::Sensors::sigma_css),
    // magnetometer
    beta_mag(Param::Sensors::beta_mag),
    sigma_mag(Param::Sensors::sigma_mag),
    // GPS
    omega_earth(Param::Earth::omega_earth),
    K_gps(Param::Sensors::K_gps),
    T_gps(Param::Sensors::T_gps),
    nu(Param::Vector3::Zero()),
    sigma_cep(Param::Sensors::sigma_gps_cep),
    sigma_V(Param::Sensors::sigma_V),
    y_gps_previous(Param::Vector6::Zero()),
    previous_gps_update(0.0),
    // reaction wheels
    omega_w_previous(Param::Orbit::InitialState.omega_wheel),
    omega_w_meas(omega_w_previous),
    sigma_w(Param::Sensors::sigma_wheel),
    alpha_w(Param::Sensors::alpha_wheel)
{
    // Initialize GPS in ECEF frame (not ECI) to match gps() output
    Scalar jd_init = helpers.julianDate(epoch_time);
    Param::Matrix3 C_eci2ecef_init = helpers.dcmeci2ecef(jd_init);
    Vector3 r_ecef_init = C_eci2ecef_init * Param::Orbit::InitialState.r_ECI;
    Vector3 v_ecef_init = C_eci2ecef_init * Param::Orbit::InitialState.v_ECI - 
...
SensorsClass::Measurements SensorsClass::measurements(const StateVector& states_dot, 
    ...
    // update sun sensor 
    Param::Vector6 y_css = sun_sensor(states);
...
SensorsClass::StateVector SensorsClass::measurements2states(const Measurements& measurements) {
    ...
    Param::Vector6 y_gps = measurements.segment<6>(19);

    Param::Matrix3 C_eci2ecef = helpers.dcmeci2ecef(helpers.julianDate(current_time));
...
SensorsClass::Quat SensorsClass::star_tracker(const StateVector& states) {
    ...
    if (star_update < random_val) {
        ...
        if (beta_star.norm() < small_angle_tol) {
            ...
            q_bias_vec = n_hat_b * std::sin(angle_b / 2.0);
            q_bias_scl = std::cos(angle_b / 2.0);
            q_beta(0) = q_bias_scl;
            q_beta(1) = q_bias_vec(0);
            q_beta(2) = q_bias_vec(1);
            q_beta(3) = q_bias_vec(2);
...
Param::Vector6 SensorsClass::sun_sensor(const StateVector& states) {
    // model sun sensor 
    Quat q = states.segment<4>(6);
    ...
    Vector3 I_p = s_b_hat.cwiseMax(0.0) * I_max;
    Vector3 I_n = (s_b_hat * -1.0).cwiseMax(0.0) * I_max;

    Param::Vector6 I_css;
    I_css.setSegment(0, I_p);
    I_css.setSegment(3, I_n);

    Param::Vector6 y_css = I_css + beta_css + sigma_css.cwiseProduct(randn<6>());
    return y_css;
}
...
SensorsClass::Vector3 SensorsClass::magnetometer(const StateVector& states) {
    ...
    Param::Matrix3 C_ned2ecef = helpers.dcmecef2ned(lla.lat, lla.lon).transpose();

    // Rotation matrix from ecef to eci 
    Param::Matrix3 C_ecef2eci = helpers.dcmeci2ecef(jd).transpose();
...
Param::Vector6 SensorsClass::gps(const StateVector& states) {
    // model gps 
    Vector3 R = states.segment<3>(0);
    ...
    nu = nu * phi + sigma_cep.cwiseProduct(randn<3>()) * std::sqrt(1 - phi * phi);
...
SensorsClass::Vector4 SensorsClass::reaction_wheels(const StateVector& states) {
    ...
    omega_w_meas = meas_raw * alpha_w + omega_w_meas * (1.0 - alpha_w);
