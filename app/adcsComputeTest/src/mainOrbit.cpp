...
#define VERBOSE_EVERY    20   /* full sensor dump every Nth cycle (~1/sec) */
 ...
 *
 *  LSM6DSV gyro @ +/-250 dps  : 8.75 mdps/LSB
 *  I3G4250D     @ +/-245 dps  : 8.75 mdps/LSB
...
int main(void)
{
	...
	/* --- ADCS Core init --- */
	printk("\n[ADCS] Initializing ADCSCore...\n");
	ADCS::Core adcs_core;
	ADCS::Command cmd;
	cmd.mode = ADCS::MissionMode::DETUMBLE;
	printk("[ADCS] Mode: DETUMBLE (B-dot controller)\n");
	printk("[ADCS] Sensors available: gyro (4x averaged), mag (3x averaged)\n");
	printk("[ADCS] Missing: star tracker, CSS, GPS, reaction wheels (zeroed)\n\n");
	...
	while (1) {
		 ...
		 *  Saves ~20ms vs sequential per-sensor waits
		 * ============================================================ */
		...
		/* Verbose: dump raw sensor data (~once per second) */
		if (verbose) {
			for (int i = 0; i < 2; i++) {
				if (imu_ok[i])
		...
		sd.star_quat     = Math::Vec<4>{1.0f, 0.0f, 0.0f, 0.0f};
		sd.css_currents  = Math::Vec<6>::Zero();
		sd.magnetometer  = Math::Vec<3>{mag_avg[0], mag_avg[1], mag_avg[2]};
		sd.gps_ecef      = Math::Vec<6>::Zero();
		sd.wheel_speeds  = Math::Vec<4>::Zero();

		ADCS::AdcsOutput out = adcs_core.update(sd, cmd);

		/* ============================================================
		 *  Print ADCS output (compact — every cycle for MATLAB viewer)
		 * ============================================================ */
