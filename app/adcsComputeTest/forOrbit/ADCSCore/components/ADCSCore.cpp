...
Core::Core()
    : observer_(), refgen_(), controller_(), first_update(true), last_time(static_cast<Param::TimeReal>(0.0))
{
}
...
AdcsOutput Core::update(const SensorData& sensors, const Command& cmd)
{
    ...
    Param::Vector17 states_hat = observer_.update(meas, sensors.unix_time, dt_scalar);

    // 3. Run reference generator with command
    auto ref_out = refgen_.update(states_hat, cmd, sensors.unix_time, dt_scalar);
    Param::Vector10 reference = ref_out.reference;
    Param::PointingMode internal_mode = ref_out.mode;

    // 4. Run controller
    auto ctrl_out = controller_.update(states_hat, reference, meas, internal_mode, dt_scalar);
