 ...
 *
 *   Your ESP32 has a UART (serial port) connected to the Orion B16 chip.
...
int orion_driver_init(orion_driver_t *drv, const orion_driver_cfg_t *cfg)
{
    ...
    /* Get UART device from config */
    drv->uart_dev = cfg->uart_dev;
    if (!drv->uart_dev) {
        LOG_ERR("No UART device provided in config");
        return -ENODEV;
    }
    ...
    LOG_INF("Orion B16 driver initialized (baud=%u)", cfg->baud_rate);
